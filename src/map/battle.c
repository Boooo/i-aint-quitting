...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv)
{
	...
	if( sc && sc->count )
	{
		...
		if( sc->data[SC_SAFETYWALL] && (flag&(BF_SHORT|BF_MAGIC))==BF_SHORT )
		{
			...
			#endif//Na renova鈬o a habilidade tambem pode ser cancelada pelo numero de hits[Sicks]
				if (--group->val2<=0)
					skill_delunitgroup(group);
				d->dmg_lv = ATK_BLOCK;
				return 0;
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		else
		{
			switch( skill_num )
			{
				...
				case AS_SPLASHER:
					#ifdef REMODE
					skillratio += 400+75*skill_lv;//Na renova鈬o o dano no nivel 10 aumentou de 1000% para 1250%[Sicks]
					#else
