...
/// void		sv_readlsqdb
// char*	name				->	nome destinado ‡ primeira DB							;
// char*	name2				->	nome destinado ‡ segunda DB								;
// int		param_size			->	n˙mero de par‚metros existentes em cada linha			;
// int		max_allowed			->	m·ximo n˙mero de entradas permitidas					;
// bool		(*sub_parse_row)	->	obtÈm par‚metros para a leitura das colunas especÌficas	;
void sv_readsqldb (char* name, char* name2, int param_size, int max_allowed, bool (*sub_parse_row)(char* string[], int columns, int current))
{
	...
	for (i = 0; i < 2; ++i)
	{
		...
		/* Caso o nome da db seja nulo, a condiÁ„o È verdadeira. */
		if (db_name[i] == NULL)
		{
			/* Se i for diferente de 1, isto È, n„o for a 2™ DB sendo lida,
			   o loop retornar· ao inÌcio, caso contr·rio ir· ser interrompido,
			   j· que ser·, ent„o, a 2™ DB e ˙ltima a ser lida. */
			if (i != 1)
				continue;
			else
				break;
		...
		/* Se o retorno da query realizada obtiver erro, a condiÁ„o È verdadeira.
		   Neste caso, a query apenas È executada para verificar a existÍncia da tabela. */
		if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT * FROM `%s`", db_name[i]))
		{
			/* Exibe mensagem de debug do SQL. */
			Sql_ShowDebug(mmysql_handle);
			
			/* Igualmente a antes, caso seja a 2™ DB lida, ir· sair do loop,
			   caso contr·rio ir· retornar ao inÌcio. */
			if (i != 1)
				continue;
			else
				break;
		...
		/* Se n„o houver colunas na tabela especÌfica, ou seja, caso sÛ exista a tabela
		   e n„o haja nenhum dado guardado nesta, n„o haver· a leitura desta DB. */
		if (Sql_NumRows(mmysql_handle) <= 0)
		{
			/* Exibe a mensagem de que a DB n„o foi lida pois n„o h· entradas. */
			ShowSQL("Table '"CL_WHITE"%s"CL_RESET"' was not read because it has 0 entries.\n", db_name[i]);
			
			/* Volta ao inÌcio do loop caso seja a 1™ DB, e saÌ caso seja a 2™. */
			if (i != 1)
				continue;
			else
				break;
		...
		/* Enquanto houver sucesso na leitura das colunas, a iteraÁ„o se mantÈm. */
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle))
		{
			...
			/* Caso a contagem de entradas for igual ao tamanho m·ximo de entradas
			   permitidas, saÌ da iteraÁ„o. */
			...
			/* Limita o tamanho da vari·vel str como o tamanho m·ximo de par‚metros. */
			str[(param_size + 1)] = '\0';

			/* IteraÁ„o que ir· pegar o valor da coluna especÌfica. */
			...
			/* Caso a leitura realizada especÌfica de cada DB seja falsa, volta ao inÌcio da iteraÁ„o. */
