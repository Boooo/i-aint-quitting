...
static int shield_penalty[CLASS_COUNT];
...
static unsigned int status_base_pc_maxhp(struct map_session_data* sd, struct status_data* status)
{
	...
	val = (unsigned int)floor(floor(val * classModifier) * (1 + status->vit * 0.01)); // fix na fÛrmula do HP ~ [ItachiSan]
...
static unsigned int status_base_pc_maxsp(struct map_session_data* sd, struct status_data *status)
{
	...
	val = (unsigned int)floor(floor(val * classModifier) * (1 + status->int_ * 0.01)); // fix na fÛrmula do SP ~ [ItachiSan]
...
#ifdef REMODE
//Na renovaÁ„o a recuperaÁ„o È de 50% [Sicks]
